...
class GoGoGyroDesktop:
    def __init__(self, width=800, height=800):
        ...
        self.width = width
        self.height = height
        self.cx, self.cy = width * 0.5, height * 0.5

        # Slightly larger overall scale compared to previous script
        self.scale_px = min(width, height) * 0.55
    ...
    def draw_ring(self, surface, ring, thickness_scale=1.0, alpha_boost=0.0):
        ...
        thickness = max(1, int(self.base_thickness * thickness_scale))
        front_alpha = max(0.0, min(1.0, self.front_alpha + alpha_boost))
        back_alpha = max(0.0, min(1.0, self.back_alpha))
        front_color = tuple(int(c * 255) for c in ring.color) + (int(front_alpha * 255),)
        back_color = tuple(int(c * 255) for c in ring.color) + (int(back_alpha * 255),)

        for x0, y0, x1, y1 in segs_back:
            pygame.draw.line(surface, back_color, (x0, y0), (x1, y1), thickness)
        for x0, y0, x1, y1 in segs_front:
            pygame.draw.line(surface, front_color, (x0, y0), (x1, y1), thickness)
    ...
    def draw(self):
        ...
        for r in self.rings:
            self.draw_ring(layer, r, thickness_scale=thickness_scale, alpha_boost=alpha_boost)
