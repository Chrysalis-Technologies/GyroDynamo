...
class SettingsStore {
  constructor(key = "gyroVisualizer.webPwa.settings.v1") {
    this.key = key;
  }
...
class HapticManager {
  constructor(settings) {
    ...
    if (!this.canVibrate) console.info("Gyroscope Visualizer haptics unavailable in this browser.");
  }
...
class OrientationNormalizer {
  ...
  normalize(orientation, requestedMode) {
    ...
    // Landscape remaps pitch/roll into screen coordinates so a sideways phone grip
    // still tilts the visual in the expected display direction.
    return {
      yaw: orientation.yaw,
...
class WebSensorManager {
  ...
  async start(onSample, onStatus) {
    ...
    this.publish(SENSOR_STATE.REQUESTING, "Requesting browser motion permission from this tap.");

    if (!("DeviceOrientationEvent" in window)) {
      this.publish(SENSOR_STATE.UNAVAILABLE, "Device orientation is unavailable in this browser. Demo Mode is available.");
      return;
    }

    try {
      const orientationPermission = await this.requestOrientationPermission();
      const motionPermission = await this.requestMotionPermission();
      if (orientationPermission === "denied" || motionPermission === "denied") {
        this.publish(SENSOR_STATE.PERMISSION_DENIED, "Motion permission was denied. Demo Mode is available.");
        return;
      }
      ...
      this.watchdog = window.setTimeout(() => {
        ...
        this.publish(
          ...
          "No motion samples arrived. iPhone Safari requires HTTPS and a real device. Demo Mode is available.",
        );
      }, 1600);
    } catch (error) {
      this.publish(SENSOR_STATE.PERMISSION_DENIED, `Motion permission failed: ${error.message || error}. Demo Mode is available.`);
    }
  }
  ...
  stop() {
    this.desiredStart = false;
    this.detach();
    this.publish(SENSOR_STATE.NOT_STARTED, "Tap Start Sensors to use iPhone motion data.");
  }
  ...
  detach(clearWatchdog = true) {
    window.removeEventListener("deviceorientation", this.handleOrientation);
    window.removeEventListener("devicemotion", this.handleMotion);
    this.isListening = false;
    if (clearWatchdog && this.watchdog) {
  ...
  handleOrientation(event) {
    ...
    this.publish(SENSOR_STATE.ACTIVE, "DeviceOrientationEvent active.");
    this.onSample?.({
      timestamp: performance.now(),
      orientation: {
        yaw: normalizeSignedDegrees(event.alpha ?? 0),
        pitch: normalizeSignedDegrees(event.beta ?? 0),
        roll: normalizeSignedDegrees(event.gamma ?? 0),
      },
      rotationRate: this.lastMotion.rotationRate,
      acceleration: this.lastMotion.acceleration,
      source: "web",
      isValid: true,
      statusMessage: "DeviceOrientationEvent",
    });
  }
...
class GyroRenderer {
  constructor(canvas) {
    ...
    this.radii = [1.0, 0.8, 0.62, 0.46, 0.34];
  ...
  render(renderState, trailSamples) {
    ...
    const center = { x: this.width * 0.5, y: this.height * 0.46 };
    const minDimension = Math.min(this.width, this.height);
    const projectionScale = minDimension * 0.42;
    const orientation = renderState.displayOrientation;
    const basePhase = (Math.PI * 2 * this.elapsed) / 30;
    ...
    this.renderGyroRings(center, projectionScale, orientation, basePhase, theme);
    this.renderMotionIndicator(center, projectionScale, orientation, theme);
    this.renderHub(center, minDimension, theme);
  }
  ...
  renderGyroRings(center, projectionScale, orientation, basePhase, theme) {
    this.radii.forEach((radius, index) => {
      const sign = index % 2 === 0 ? 1 : -1;
      const spin = sign * basePhase * (index + 1);
  ...
  renderDepthRing(config) {
    ...
    const drawPass = (frontPass) => {
      for (let index = 0; index < points.length - 1; index += 1) {
        ...
        const lineWidth = config.theme.lineWeight * (frontPass ? 0.8 + depth * 0.88 : 0.38 + depth * 0.36);
  ...
  renderMotionIndicator(center, projectionScale, orientation, theme) {
    ...
    this.ctx.beginPath();
    this.ctx.arc(projected.x, projected.y, 14, 0, Math.PI * 2);
    this.ctx.fill();
    this.ctx.fillStyle = colorToRgba(theme.accent, 0.92);
    this.ctx.beginPath();
    this.ctx.arc(projected.x, projected.y, 5.5, 0, Math.PI * 2);
    this.ctx.fill();
    this.ctx.restore();
  }

  renderHub(center, minDimension, theme) {
    const radius = Math.max(14, minDimension * 0.026);
    this.ctx.save();
    this.ctx.shadowColor = theme.accent;
...
class GyroApp {
  constructor() {
    ...
    this.sensorStatus = {
      ...
      source: "unavailable",
      message: "Tap Start Sensors to use iPhone motion data.",
  ...
  async startSensors() {
    ...
    this.sensorManager = new WebSensorManager();
    this.haptics.lightImpact();
    console.info("Gyroscope Visualizer sensor start requested.");
  ...
  enableDemoMode() {
    ...
    this.haptics.modeChanged();
    console.info("Gyroscope Visualizer demo mode enabled.");
  ...
  stopSensors() {
    ...
    console.info("Gyroscope Visualizer sensor stop requested.");
  }
  ...
  calibrate() {
    if (!this.lastNormalized) {
      this.handleStatus({
        ...this.sensorStatus,
        message: "No current orientation sample. Start Sensors or Demo Mode before calibrating.",
      });
    ...
    this.haptics.resetThresholdState();
    console.info("Gyroscope Visualizer calibration set.");
  ...
  handleStatus(status) {
    ...
    if (status.state !== this.lastLoggedState) {
      ...
      console.info(`Gyroscope Visualizer status: ${SENSOR_STATE_TITLE[status.state]} - ${status.message}`);
    }
  ...
  updateUI() {
    ...
    this.elements.drawerStatus.textContent = `${title} · ${theme.title} · ${this.titleFor("trailLength", this.settings.trailLength)} trails`;
  ...
  diagnosticsLine() {
    ...
    return `src ${this.sensorStatus.source} · interval ${this.sensorStatus.updateIntervalMs.toFixed(1)} ms · age ${age} · ${late} · haptics ${this.haptics.adapterType}`;
  }
  ...
  async registerServiceWorker() {
    ...
    try {
      ...
      console.info(`Gyroscope Visualizer service worker unavailable: ${error.message || error}`);
    }
  }
