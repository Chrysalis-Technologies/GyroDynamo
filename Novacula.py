...
def build_scene():
    global camera_pivot, camera_node
...
class MainView(ui.View):
    def __init__(self):
        ...
        self.overlay.multiple_touch_enabled = True
    ...
    def update_camera(self):
        self.camera_pivot.eulerAngles = (self.pitch,self.yaw,0)
        self.camera_node.position = (0,0,self.dist)

    def touch_began(self, touch):
        self.active_touches[touch.touch_id] = touch
        ...
        if len(self.active_touches) == 1:
            ...
            t1,t2 = list(self.active_touches.values())
    ...
    def touch_ended(self, touch):
        ...
        if len(self.active_touches) == 1:
            remaining = next(iter(self.active_touches.values()))
            self.last_pt = remaining.location
        else:
            self.last_pt = None

    def update_emission(self):
        global emission_phase
        emission_phase += 0.05
        intensity = 0.8 + 0.2*math.sin(emission_phase)
