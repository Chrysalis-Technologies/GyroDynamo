...
set "SCRIPT=%ROOT%GoGoGyroUniverse.py"
set "CONFIG=%ROOT%universe.json"

if not exist "%PY%" (
  echo [GoGoGyro] Missing venv: "%PY%"
  echo Create it with: py -3 -m venv .venv-win
  echo Then install deps:
  echo   .venv-win\Scripts\python.exe -m pip install pygame
  echo   .venv-win\Scripts\python.exe -m pip install pillow   ^(only if using --serve-mjpeg^)
  pause
  exit /b 1
)
...
if not exist "%CONFIG%" (
  echo [GoGoGyro] Missing config: "%CONFIG%"
  echo Create it by copying the template universe.json into this folder.
  pause
  exit /b 1
)
